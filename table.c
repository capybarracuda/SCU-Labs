...
int partition(void **a, int lo, int hi, int(*cmp)())//O(n) partitions pivot so that entire left half is less and right half is more
{
	void* piv=a[hi];
	void* temp;
	int j=lo-1,i;
...
void quicksort(void **a, int lo, int hi, int(*cmp)())//O(nlogn) sorts partitions recursively
{
	int pi;
	if(lo<hi)
	{
		pi=partition(a,lo,hi,cmp);
		
		quicksort(a,lo,pi-1,cmp);
		quicksort(a,pi+1,hi,cmp);
	}
}

/*
 *Function: *getElements(SET *sp)
 *
 *Complexity:O(nlogn)
 *
 *Description: returns sorted array of elts
 *
 */
